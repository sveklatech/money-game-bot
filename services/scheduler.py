...
from database.database import get_all_users, get_user, get_stats
...
async def send_morning_notification(context):
    ...
    for user in users:
        try:
            ...
            if h == current_hour and abs(m - current_minute) <= 1:
                ...
                if current_day and 1 <= current_day <= 30:
                    ...
                    if day_data:
                        day_text = (
                            ...
                            f"_{day_data['task'][:200]}..._" if len(day_data['task']) > 200
                            else f"\n\n📅 *Сегодня — День {current_day}: {day_data['title']}*\n_{day_data['task']}_"
                        )
